<script>
  ...
  const appRef = { _ref: 'application', _enum: 'ordinal', _value: 'targetEnum' };
  ...
  async function performMenuCommand(commandId){
    ...
    await core.getMenuCommandState({ commandID: commandId })
      .then((res) => {
        /*
        uxpLog(2, res)
  ...
  async function buildCommandCollection(){
    ...
    function crawlSubmenu(name, array){
      ...
      array.forEach(element => {
        ...
        if(element.submenu){
          let nextName = '';
          if(name == ''){ // name == '' if it's starting the recursion
          ...
          commandCollection.push({category: name, name: element.name, enabled: element.enabled, visible: element.visible, command: element.command});
        }     
      });
    }
      ...
      switch (EDITOR_PACKET["event"]) {
        case "message":
          if(EDITOR_PACKET.data != undefined){
            const decodedMessage = JSON.parse(decodeBase64(EDITOR_PACKET.data));
